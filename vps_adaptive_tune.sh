...
# vps_adaptive_tune.sh
# Adaptive Ubuntu VPS tuning for OCI (Oracle Cloud) and generic cloud VMs.
# It inspects system resources/config first, then decides what to apply.
#
# Modes:
#   MODE=apply (default)  Apply changes
#   MODE=plan             Print decisions only (no changes)
#
# Optional env vars (override decisions):
#   APPLY_SYSCTL=auto|0|1
#   APPLY_JOURNALD=auto|0|1
#   APPLY_FSTRIM=auto|0|1
#   APPLY_SYSTEMD_LIMITS=auto|0|1
#   APPLY_PAM_LIMITS=auto|0|1
#   APPLY_DISABLE=auto|0|1
#   APPLY_DOCKER=auto|0|1
#   APPLY_BBR=auto|0|1
#   APPLY_ZRAM=auto|0|1
#   APPLY_SERVICE_AUDIT=auto|0|1
#   APPLY_DISABLE_CANDIDATES=auto|0|1
#   APPLY_PURGE_CANDIDATE_PACKAGES=auto|0|1
#
#   INSTALL_ZRAM=0|1      If APPLY_ZRAM=1 but generator missing, try to install
#
# Risk toggles:
...
log() { echo "[vps-adaptive] $*"; }
warn() { echo "[vps-adaptive][WARN] $*" >&2; }
...
to_bool() {
  # Convert auto/0/1 into 0/1 using a default.
...
detect_oci() {
  # Heuristics: DMI strings or presence of OCI metadata endpoint.
...
detect_docker() {
  if have_cmd docker; then echo 1; else echo 0; fi
}
...
list_running_services() {
  # Prints service unit names only.
...
decide_defaults() {
  ...
  # Process limits: conservative defaults.
  ...
  # Decide toggles (auto => chosen defaults).
  ...
  # ZRAM: only auto-enable for small-memory nodes.
  ...
  # Service audit is safe to run by default.
  DO_SERVICE_AUDIT="$(to_bool "$APPLY_SERVICE_AUDIT" 1)"
  # Disabling/purging candidates is potentially disruptive: default off unless explicitly enabled.
...
print_report_and_plan() {
  ...
  echo " - systemd limits: ${DO_SYSTEMD_LIMITS} (DefaultLimitNOFILE=${LIMIT_NOFILE_DEFAULT}, DefaultTasksMax=${TASKS_MAX_DEFAULT})"
  echo " - pam limits: ${DO_PAM_LIMITS} (* nofile ${LIMIT_NOFILE_DEFAULT})"
  echo " - disable services: ${DO_DISABLE}"
...
apply_sysctl() {
  ...
  write_file_if_changed "/etc/sysctl.d/99-vps-adaptive.conf" "# Managed by vps_adaptive_tune.sh\n\
vm.swappiness=10\n\
...
apply_journald() {
  ...
  write_file_if_changed "/etc/systemd/journald.conf.d/99-vps-adaptive.conf" "[Journal]\n\
...
apply_systemd_limits() {
  ...
  write_file_if_changed "/etc/systemd/system.conf.d/99-vps-adaptive.conf" "[Manager]\n\
...
apply_pam_limits() {
  ...
  write_file_if_changed "/etc/security/limits.d/99-vps-adaptive.conf" "# Managed by vps_adaptive_tune.sh\n\
...
candidate_units() {
  # Keep this list conservative. These are commonly useless on headless cloud VPS.
  # NOTE: Do not include cloud-init/systemd-networkd/ssh/cron/resolved/timesyncd.
  cat <<'EOF'
multipathd.service
open-iscsi.service
iscsid.service
...
print_service_audit() {
  ...
  log "Service audit (running services + conservative candidates):"
  local running
  running="$(list_running_services || true)"
  if [[ -z "$running" ]]; then
    echo " - running: (none detected or systemctl unavailable)"
  else
  ...
  # Snap candidates are unit-named, but removal is snap-specific; only report.
...
disable_candidate_services() {
  ...
  log "Disabling candidate services (explicitly enabled):"
  ...
  # Snap units: only stop/disable, no snap removal.
  disable_service_if_exists "snap.lxd.daemon.service"
  disable_service_if_exists "snap.lxd.activate.service"
  disable_service_if_exists "snap.cups.cupsd.service"
}
...
purge_candidate_packages() {
  ...
  log "Purging candidate packages (explicitly enabled):"
  # Map services to packages conservatively.
  # Note: snap-managed software (like lxd/cups snap) should be removed via snap.
  local pkgs=()
  if unit_exists "multipathd.service"; then pkgs+=(multipath-tools); fi
  if unit_exists "open-iscsi.service" || unit_exists "iscsid.service"; then pkgs+=(open-iscsi); fi
  if unit_exists "cups.service" || unit_exists "cups-browsed.service"; then pkgs+=(cups cups-browsed); fi
  if unit_exists "avahi-daemon.service"; then pkgs+=(avahi-daemon); fi
  if unit_exists "bluetooth.service"; then pkgs+=(bluez); fi
  if unit_exists "ModemManager.service"; then pkgs+=(modemmanager); fi
  if unit_exists "rpcbind.service"; then pkgs+=(rpcbind); fi
  if unit_exists "nfs-server.service"; then pkgs+=(nfs-kernel-server); fi

  if (( ${#pkgs[@]} == 0 )); then
    log " - no candidate packages detected"
    return 0
  fi
...
apply_bbr() {
  [[ "$DO_BBR" == "1" ]] || return 0
  mkdir -p /etc/sysctl.d
  write_file_if_changed "/etc/sysctl.d/99-vps-adaptive-net.conf" "# Managed by vps_adaptive_tune.sh\n\
...
install_zram_generator_if_needed() {
  ...
  # Try common package names; best-effort.
  apt-get -y update >/dev/null 2>&1 || true
...
apply_zram() {
  ...
  if [[ "$HAS_ZRAM_GEN" != "1" ]]; then
    warn "ZRAM requested but zram-generator not found; skipping. Set INSTALL_ZRAM=1 to attempt install."
    return 0
  fi

  # Conservative zram sizing: 25% of RAM.
  mkdir -p /etc/systemd
  write_file_if_changed "/etc/systemd/zram-generator.conf" "# Managed by vps_adaptive_tune.sh\n\
[zram0]\n\
zram-size = ram / 4\n\
...
swap-priority = 100\n"
  ...
  systemctl daemon-reload >/dev/null 2>&1 || true
  if systemctl list-unit-files | awk '{print $1}' | grep -qx 'systemd-zram-setup@.service'; then
...
# Helpful for production: keep containers running during daemon restart.
data.setdefault("live-restore", True)

p.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="utf-8")
print("[vps-adaptive] Wrote /etc/docker/daemon.json (log rotation + live-restore)")
PY
}
